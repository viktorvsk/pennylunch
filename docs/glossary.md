...
Notes: Vectors are nullable and generated by `IndexRecipeJob` after import.

## Recipe Ingredient

Definition: A resolved association between one Recipe and one catalog Ingredient.
Owner: Ingredient search.
Notes: Stored in `recipe_ingredients` with required `recipe_id` and `ingredient_id`. Rows are recomputed by `IndexRecipeJob` from `recipes.ingredient_names` through the Ingredient alias catalog. Unknown parsed recipe ingredient names do not create rows.
