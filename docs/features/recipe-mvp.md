...
The panel gets autocomplete options from canonical `Ingredient.name` values plus optional metadata. Users add ingredients from a bottom autocomplete input by choosing a dropdown match or pressing Enter on the active match. Only listed options can be added, exact and prefix matches rank before substring matches, and already-selected names are removed from the suggestions. The selected-ingredients surface keeps the color pen mark centered as a persistent background; when the Market basket is empty, it also shows the empty-state phrase below the mark. Selected ingredients render as a removable row list with required ingredients first and optional ingredients after them. Optional ingredients render as visibly secondary and are saved in the basket, but they are omitted from the hidden filter field and never affect ingredient search. The application layout emits a short-lived cached recipe UI catalog containing ingredient options, category labels, and category slugs; Stimulus modules read that catalog for FAB autocomplete and category URL construction. The panel stores selected names and enabled state in the `pennylunch.ingredients` cookie so Rails can apply the basket during the first index render when the URL has no explicit `ingredients` parameter. Existing `localStorage` baskets are migrated to the cookie on the next client load. Panel visibility remains transient: outside clicks, normal Turbo navigation, and page refreshes close it. Ingredient-panel filter submissions target the `recipe-results-frame` Turbo Frame, and the FAB is marked `data-turbo-permanent`, so result updates do not replace or close the FAB. On full Turbo navigation, the preserved FAB refreshes against the new toolbar form so the On state and hidden `ingredients` field stay aligned without issuing a catch-up filter submit. The switch can be on or off independently of whether any ingredients are selected, disabled selections remain saved but are omitted from filter URLs, and enabled non-empty non-optional selections are written to the hidden field and included in Turbo filter navigation. The enable control and active FAB state use the herb theme. The shared FAB is rendered from the application layout.

The bottom ingredient input includes a joined photo scan button with AI sparkles and camera icons. It opens a native modal with a large drag-and-drop photo target plus a direct image URL field; choosing or dropping a JPEG, PNG, WebP, or GIF photo starts reading immediately, and pasting a URL starts reading without a separate submit button. While reading, the modal swaps to a quiet AI-thinking spinner with subtle star motion. The server endpoint `POST /ingredient_image` validates either source, converts uploads into base64 image data URLs, and passes public remote image URLs directly to OpenRouter. `ImageReader` calls OpenRouter's `google/gemini-2.5-flash-lite` model with `google/gemini-2.5-flash` as a fallback and a forced function call constrained by the current canonical `Ingredient.name` prompt. The JSON response returns only catalog names that were recognized from the image, and the Stimulus FAB controller merges them into the basket cookie and visible row list. If the basket switch is already On, the existing Turbo filter submission path updates results; if it is Off, the detected ingredients are saved but not applied to filtering until the user turns the switch On.
