...
The MVP has a simple, inspectable data model and a strong import equality spec. Re-importing changed source data is intentionally not handled yet. Ingredient relevance depends on the Ingredient catalog, recomputed `RecipeIngredient` rows, and vectors generated through Avo-queued recipe indexing jobs. Parser output, recipe ingredient associations, and vectors are derived data, so they can be regenerated without changing the original source fields or raw parser ingredient names.
