...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y curl libjemalloc2 libvips postgresql-client python-is-python3 python3 python3-venv && \
...
RUN python -m venv /rails/.venv && \
    hash -r && \
    python -m pip install --upgrade pip && \
    python -m pip install --no-cache-dir -r requirements.txt && \
    python -c "import nltk, os; nltk.download('averaged_perceptron_tagger_eng', download_dir=os.environ['NLTK_DATA'], quiet=True); from ingredient_parser import parse_ingredient; parse_ingredient('1 cup flour')"
